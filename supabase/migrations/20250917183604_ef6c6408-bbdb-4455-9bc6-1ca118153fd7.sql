...
-- First, drop existing RLS policies that reference these columns
...
-- Add foreign key constraints to reference members.member_id (which is already int)
ALTER TABLE public.img_links 
...
-- Recreate RLS policies with updated logic using member_id instead of UUID lookups
